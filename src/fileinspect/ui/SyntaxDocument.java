...
public class SyntaxDocument extends DefaultStyledDocument {
	...
	private MutableAttributeSet table;
	...
	public SyntaxDocument() {
		...
		StyleConstants.setForeground(comment, new Color(78, 154, 69));
		...
		table = new SimpleAttributeSet();
		//StyleConstants.setForeground(table, new Color(134, 117, 23));
		//StyleConstants.setForeground(table, new Color(95, 58, 58));
		//StyleConstants.setItalic(table, true);
	...
	public void setTableSet(HashSet aHashSet) {
		tables = aHashSet;
	}
	...
	private int getOtherToken(String content, int startOffset, int endOffset) {
		...
		if (isTable(token)) {
			...
			//      try{
			//      doc.replace(startOffset,token.length(),upper,null);
			//      }catch (BadLocationException e){
			//        
			//      }
			doc.setCharacterAttributes(startOffset, endOfToken - startOffset, table, false);
		}
	...
	protected boolean isTable(String token) {
		if (tables != null) {
			return tables.contains(token);
		} else {
			return false;
		}
	}
